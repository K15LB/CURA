...
import { listCustomerStatuss } from '../graphql/queries';

class CustomerStatus extends Component {
    render() {
        const CustomerStatusDetail = (props) => {
            const customerstatus = props.data.listCustomerStatuss.items;
            return customerstatus.map((data) => {
        ...
        return (
            <Query query={gql(listCustomerStatuss)}>
                {({ data, loading, error }) => {
                    if (error) return (<p>Error</p>);
                    if (loading || !listCustomerStatuss) return (<p>Loading...</p>);
                    console.log(data.listCustomerStatuss)
                    return (
                        <CustomerStatusDetail data={data} />
                    )
                }}
            </Query>
        )
